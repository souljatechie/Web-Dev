...
if ( !empty($_REQUEST['ajax']) ) {
	...
	case 'photo_js': ?>
		...
		for (i = 0; i < my_src.length; i++) {
			...
			maybeappend = '<a href="../../testing/wordpress/wp-admin/?ajax=photo_thickbox&i=' + encodeURIComponent(img.src) + '&u=<?php echo urlencode($url); ?>&height=400&width=500" title="" class="thickbox"><img src="' + img.src + '" ' + img_attr + '/></a>';
		...
		function pick(img, desc) {
			if (img) {
				...
				insert_editor( "\n\n" + encodeURI('<p style="text-align: center;"><a href="<?php echo $url; ?>"><img src="../../testing/wordpress/wp-admin/' + img +'" alt="' + desc + '" /></a></p>'));
			}
			return false;
		}
