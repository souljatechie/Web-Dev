...
?>
		<ol>
			<li><p><?php printf( __( 'Add the following to your <code>%s</code> file:' ), DEBIAN_FILE ); ?></p>
				<textarea class="code" readonly cols="100" rows="6">
...
<configuration>
	...
	?>
		...
		?>
		<textarea class="code" readonly cols="100" rows="20"><?php echo esc_textarea( $web_config_file ); ?>
		</textarea></li>
		</ol>
...
EOF;
		...
		if ( ! $subdomain_install && WP_CONTENT_DIR != ABSPATH . 'wp-content' )
			echo '<p><strong>' . __('Warning:') . ' ' . __( 'Subdirectory networks may not be fully compatible with custom wp-content directories.' ) . '</strong></p>';
		?>
		<textarea class="code" readonly cols="100" rows="<?php echo substr_count( $htaccess_file, "\n" ) + 1; ?>">
