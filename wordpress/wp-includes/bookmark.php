...
function get_bookmark($bookmark, $output = OBJECT, $filter = 'raw') {
	...
	if ( empty($bookmark) ) {
		...
		if ( isset($GLOBALS['link']) && ($GLOBALS['link']->link_id == $bookmark) ) {
			...
			if ( $_bookmark ) {
				$_bookmark->link_category = array_unique( wp_get_object_terms( $_bookmark->link_id, 'link_category', array( 'fields'../../testing/wordpress/wp-includes/ => 'ids' ) ) );
...
function get_bookmarks($args = '') {
	...
	$defaults = array(
		...
		'limit' => -1, 'category'../../testing/wordpress/wp-includes/ => '',
		'category_name'../../testing/wordpress/wp-includes/ => '', 'hide_invisible' => 1,
		'show_updated' => 0, 'include'../../testing/wordpress/wp-includes/ => '',
		'exclude' => '', 'search' => ''
	);
